(() => {
    ...
    function menuInit() {
        if (document.querySelector(".icon-menu")) document.addEventListener("click", (function(e) {
            if (bodyLockStatus && e.target.closest(".icon-menu")) {
                const catalogM = document.querySelector("._show");
                const catalogMSub = document.querySelector("._submenu-open");
                bodyLockToggle();
                document.documentElement.classList.toggle("menu-open");
                catalogM ? catalogM.classList.remove("_show") : null;
                catalogMSub ? catalogMSub.classList.remove("_submenu-open") : null;
            }
        }));
    }
    ...
    function initSliders() {
        if (document.querySelector(".catalog-hero__body")) new core(".catalog-hero__body", {
        ...
        if (document.querySelector(".images-product__slider")) {
            const thumbsSlider = new core(".images-product__thumbs", {
    ...
    if (document.documentElement.closest(".touch")) document.addEventListener("click", documentActionsMenu);
    function documentHover(e) {
        ...
        if (targetElement.closest("[data-parent]")) {
            ...
            if (subMenu) {
                ...
                targetElement.classList.add("_submenu-open--active");
                subMenu.classList.add("_submenu-open");
            } else console.log("Нет такого меню!");
            e.preventDefault();
        }
        if (!targetElement.closest("[data-parent]") && !targetElement.closest("[data-submenu]")) if (document.documentElement.clientWidth > 768) {
            const activeLink = document.querySelector("._submenu-open--active"), activeBlock = document.querySelector("._submenu-open");
            activeLink ? activeLink.classList.remove("_submenu-open--active") : null;
            activeBlock ? activeBlock.classList.remove("_submenu-open") : null;
        }
    ...
    function documentActions(e) {
        const targetElement = e.target;
        if (targetElement.closest(".middle-header__search-btn")) {
            const headerForm = document.querySelector(".middle-header__form");
            headerForm.classList.toggle("_show");
        }
        if (targetElement.closest(".catalog-menu__link_m")) {
            const subMenu = document.querySelector(".catalog-menu__submenu");
            subMenu.classList.add("_show");
        }
        if (targetElement.closest(".submenu__back_up")) document.querySelector("._show").classList.remove("_show");
        if (targetElement.closest(".submenu__back_down")) document.querySelector("._submenu-open").classList.remove("_submenu-open");
        if (targetElement.closest(".favorite-btn")) targetElement.classList.toggle("_active-fav");
        if (targetElement.closest(".filter-btn")) {
            targetElement.classList.toggle("_active-filter");
            document.querySelector(".content-catalog__sort").classList.toggle("_show__active-filter");
        }
        if (targetElement.closest(".filter__title_m")) {
            const activeBlock = document.querySelector(".content-catalog__menu_m");
            activeBlock.classList.toggle("_show-catalog");
            targetElement.classList.toggle("_show-catalog__link");
        }
        if (targetElement.closest(".content-product__btn")) targetElement.classList.toggle("_success");
    }
    function documentActionsMenu(e) {
        const targetElement = e.target;
        if (targetElement.closest("[data-parent]")) {
            const numMenu = targetElement.dataset.parent ? targetElement.dataset.parent : null, subMenu = document.querySelector(`[data-submenu="${numMenu}"]`);
            if (subMenu) {
    ...
    if (document.querySelector(".catalog__filter")) {
        document.querySelector(".catalog__filter .filter__item:first-child .filter__title").classList.add("_spoller-active");
        document.querySelector(".catalog__filter .filter__item:first-child .spollers__title").classList.add("_spoller-active");
    }
    window["FLS"] = false;
    isWebp();
    addLoadedClass();
